...
13. **盲区优先 (v0.3.31+，强约束)**: 如果 user 消息里给了 `<covered_topic_groups>` 块，
   表示这些 topic_group 在用户推荐池里已经堆积。本轮**所有 domain 都不允许命中**这些方向 —
   即使是用户高权重兴趣。对每个生成的 domain，你必须检查它的 query 命中后内容会被 evaluator
   打成什么 topic_group：如果会落到 covered list 里，**换一个方向**。
   例：covered_topic_groups 包含 "认知科学" → 不要出 "心智哲学/思维模型/元认知" 这类会被
   合并到"认知科学"的 domain；改去 "声音设计 / 城市民俗 / 工业纪录" 等还没被覆盖的轴。
</rules>
...
""".strip()
    ...
    if covered_topic_groups:
        # Deduplicate + cap to keep the prompt size sane while still
        # carrying enough signal. Top 30 already-saturated topics is
        # plenty of avoidance signal — beyond that the model gets
        # paralyzed instead of explorative.
        seen: set[str] = set()
        ...
        for label in covered_topic_groups:
            ...
            if len(unique_covered) >= 30:
                break
        if unique_covered:
            user_prompt_parts.extend([
                "<covered_topic_groups>",
                "下面 N 个 topic_group 在用户当前推荐池里已经有足够内容,本轮 explore "
                "**禁止**生成命中这些方向的 domain / query。即使你判断这些是用户喜好的核心领域,"
                "本次也要避开 — 探索的价值在于带新东西进来,重复已覆盖区域的开销大于收益。",
