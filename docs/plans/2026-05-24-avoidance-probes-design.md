...
3. 达到 `confirmation_threshold` 后进入 promote，但 `AvoidanceSpeculator` 保持无 IO 副作用，只返回 `tick_result.promoted`。
...
2. `confirm` 将 probe 转为 confirmed，并调用统一的 dislike writeback helper 写入 flat preference 的 `disliked_topics`。这是避雷画像的持久化源头；不要只 mutate `await soul_engine.get_profile()` 返回的快照。
