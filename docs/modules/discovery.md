...
- **MusicMarkSyncService** — MusicMark 不属于 discovery adapter。它按 `[sources.musicmark]` 低频拉取听歌统计摘要，生成少量 `source_platform="musicmark"` 的画像行为事件，只影响 memory / soul，不进入 `content_cache`，也不占用候选池来源配比。
