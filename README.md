...
- **📱 手机 Web 操作台** —— 新增 `web/` Vite + TypeScript 前端，手机浏览器可直接查看推荐、画像、聊天、消息和运行状态。
...
│   扩展: 行为采集 · Cookie 同步 · xhs/dy/yt 任务调度 · 初始化导入    │
│   Web : 推荐流 · 画像 · durable 聊天 · 消息/探针 · 运行状态        │
└──────────────────────────┬──────────────────────────────────┘
                           │ REST API / WebSocket（presence + runtime-stream）
┌──────────────────────────▼──────────────────────────────────┐
│                      Agent 编排层                            │
│                (Skill 系统 · 对话管理 · Runtime Gate)         │
├──────────┬──────────┬───────────┬────────────────────────────┤
│  Soul    │ Memory   │ Discovery │  Recommendation            │
│  Engine  │ System   │  Engine   │     Engine                  │
│(满意度过滤)│ (五层)   │(负样本锚点)│   (跨源混排)                │
├──────────┴──────────┴───────────┴────────────────────────────┤
│       LLM 适配层 · B 站 API · 扩展代理发现 · Runtime Gate        │
...
发现结果经过多维度多样性选择：平台族预留配额（默认 B 站 / 小红书 / 抖音 / YouTube = 8 / 1 / 1 / 1，可通过 `[scheduler.pool_source_shares]` 配置）→ 主题去重 → 风格均衡 → **跨平台混排** → 上限封顶，确保最终推荐不局限于单一平台。B 站四策略统一计入 `bilibili`，小红书统一计入 `xiaohongshu`，抖音 search / hot / feed 统一计入 `douyin`，YouTube `yt_search` / `yt_trending` / `yt_channel` 统一计入 `youtube`；被关闭的平台会从有效配比中剔除。
...
├── web/                       # 手机 Web 操作台 (Vite + TypeScript + hash route)
├── skills/                    # 内置 Skill 定义
├── docs/                      # 项目文档
└── tests/                     # 测试 (800+)
```
...
| 手机 Web 前端 | Vite + TypeScript + 原生 DOM |
...
- [手机 Web 前端](docs/modules/web.md) — 移动端操作台、API 边界与部署脚本
...
最新版本：**v0.3.78: 手机 Web 前端操作台（2026-05-19）**。README 顶部保留最新重要更新；完整历史见 [docs/changelog.md](docs/changelog.md)，发布包见 [GitHub Releases](https://github.com/whiteguo233/OpenBiliClaw/releases)。
