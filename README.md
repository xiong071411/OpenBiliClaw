...
- 🔮 **挑战式兴趣探针** — 基于心理学桥接逻辑主动猜测你可能喜欢的未知领域，按 near/lateral/bridge/wildcard 控制距离，弱正向先进入短期 buffer，避免一次确认后短期刷屏
...
┌─────────────────────────────────────────────────────────────┐
│                       Chrome Extension                       │
│        (行为采集 · 停留满意度 · 推荐展示 · 真实可换库存 · 对话 · 探针) │
│        (Cookie 同步 · xhs/dy/yt 任务调度 · 初始化画像导入)        │
└──────────────────────────┬──────────────────────────────────┘
                           │ REST API / WebSocket（presence + Cookie 同步 + 可换库存 + 探针）
┌──────────────────────────▼──────────────────────────────────┐
│                      Agent 编排层                            │
