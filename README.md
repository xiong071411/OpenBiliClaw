...
- **🔮 挑战式兴趣探针** —— 兴趣探针按 near / lateral / bridge / wildcard 四档控制探索距离；普通 near 池最多 5 条，挑战池单独最多 3 条，不再互相挤占。
...
- **🎧 本地 MusicMark 集成** —— 本地集成分支新增 `[sources.musicmark]`，只同步聚合听歌摘要进入画像，不进入候选池；移动 Web 画像页可查看同步状态。
...
│(画像+探针)│ (五层+buffer)│(负样本锚点)│ (跨源混排+放大保护)          │
├──────────┴──────────┴───────────┴────────────────────────────┤
...
| **MusicMark** | 聚合听歌摘要同步 | 只补充画像事件，不进入候选池，不占用平台配比 |
...
前端展示的候选池数字以 runtime status 为准：`pool_available_count` 只表示已经有推荐文案、分类、可跳转链接且未进入近期已看窗口的可换内容；`pool_pending_count` 表示已经发现但还在整理的素材。插件、移动端 Web 和桌面 Web 都不会把 pending 数量展示成“可换”。
...
│   ├── runtime/               # 后台刷新、presence gate、speculator one-shot、降级 RuntimeContext
